...
BOOL ShortcutsDlg::OnInitDialog()
{
	...
	for (int i=0;i<shortcuts.GetCount();i++) {
		...
		((CComboBox*)GetDlgItem(IDC_SHORTCUTS_COMBO_SHORTCUT1_TYPE+i*3))->SetCurSel(shortcut.type);
	}
	return TRUE;
}
...
void ShortcutsDlg::OnBnClickedOk()
{
	...
	for (int i=0; i<8; i++) {
		...
		shortcut.type = (msip_shortcut_type)((CComboBox*)GetDlgItem(IDC_SHORTCUTS_COMBO_SHORTCUT1_TYPE+i*3))->GetCurSel();
		if (shortcut.type>=0 && !shortcut.label.IsEmpty() && !shortcut.number.IsEmpty()) {
